...
scale = 10;%randi([10 15],1,1);                                                % parameter to control the blocksize
...
semilogy([0:(size(MSE2,2)-1)],MSE2,'->y','linewidth',1.5);
...
figure(2)
semilogy([0:(size(error,2)-1)],error,'-sb','linewidth',1.5);hold on
semilogy([0:(size(error2,2)-1)],error2,'->y','linewidth',1.5);
legend('BrasCP accel','BrasCP');
xlabel('no. of MTTKRP computed')
ylabel('error')
set(gca,'fontsize',14)

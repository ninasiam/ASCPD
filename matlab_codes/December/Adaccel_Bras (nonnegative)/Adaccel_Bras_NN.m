...
while(1)
    ...
    %% Adaptive step size (AdaBras)
    H = kr(A_est{kr_idx(2)},A_est{kr_idx(1)});                             % khatri rao product
    G_n = (1/B(n))*(A_est{n}*H(F_n,:)'*H(F_n,:) - T_s'*H(F_n,:));          % derivative matrix 
    
    G_n_squared = G_n.^2;
    
    if iter == 1
       sum_G_n1 =  zeros(size(G_n_squared));                               % |
       sum_G_n2 =  zeros(size(G_n_squared));                               % | Hold the sum of derivative matrices
    ...
    if n == 1
        ...
        A_est_next = max(0,A_est{n} - eta_r1.*G_n);                        % projected S.Gradient step 
    elseif n == 2
        sum_G_n2 = sum_G_n2 + G_n_squared;
        eta_r2 = eta./ ((beta + sum_G_n2).^(1/2 + epsilon));
        A_est_next = max(0,A_est{n} - eta_r2.*G_n);                        % projected S.Gradient step
    elseif n == 3
        sum_G_n3 = sum_G_n3 + G_n_squared;
        eta_r3 = eta./ ((beta + sum_G_n3).^(1/2 + epsilon));
        A_est_next = max(0,A_est{n} - eta_r3.*G_n);                        % projected S.Gradient step
    end
    
    A_est{n} = A_est_next;                                                 % update the factor list
    ...
    if(mod(iter,iter_per_epoch) == 0 && iter > 0)
        ...
        legend('Adagrad Bras','Bras', 'AdaBras accel')
        xlabel('epochs');
        ylabel('error');
        grid on;
        pause(0.001)

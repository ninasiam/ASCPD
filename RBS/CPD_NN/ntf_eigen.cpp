...
int main(int argc, char **argv){
	...
	double start_t_sampl, stop_t_sampl = 0;
	...
	for(int i=0; i<3; i++){
		// BZ(i) = max(2 * R , int(4 * sqrt(DIMS(i))));
		// BZ(i) = DIMS(i)/4;
		BZ(i) = 2*DIMS(i) / 5;
	}
	cout << "Blocksize=\n" <<  BZ << endl;
	MatrixXi rand_indices(3, max(I,max(J,K)));
	rand_indices.setZero();
	MatrixXi B_cal(3, max(BZ(0), max(BZ(1),BZ(2))));
	B_cal.setZero();
	...
	while(1){	//	<-------------------------------------------	Begin AO Algorithm		------------------------------------------>	//
		...
		start_t_Wj1 = tic();
		Workerjob(omp_var, BZ(1), BZ(2), A_sub, C_T_C, B_T_B, X_A_sub, KhatriRao_CB, delta_1, delta_2, 1);
		...
		start_t_Wj2 = tic();
		Workerjob(omp_var, BZ(0), BZ(2), B_sub, C_T_C, A_T_A, X_A_sub, KhatriRao_CA, delta_1, delta_2, 2);
		...
		start_t_Wj3 = tic();
		Workerjob_C(omp_var, BZ(0), BZ(1), C_sub, W_C_sub, B_T_B, A_T_A, X_C_sub, KhatriRao_BA, delta_1, delta_2);
		...
		create_subfactors(C, C_sub, X_C, X_C_sub, DIMS, BZ, rand_indices, B_cal, AO_iter, 3);
		// std::cout << "\n\nX_C_sub = \n" << X_C_sub << "\n\n\n";
		stop_t_sampl += toc(start_t_sampl);
	...
	cout << " <Sampling>              \t|\tCPU time = " << std::setprecision(5) << stop_t_sampl << "\t|\t" << std::setprecision(5) << 100 * stop_t_sampl / stop_t << " % (Total)" << endl;

	double leftover_time = stop_t_KCB + stop_t_ATA + stop_t_Wj1 + stop_t_KCA + stop_t_BTB + stop_t_Wj2 + stop_t_KBA + stop_t_CTC + stop_t_Wj3 + stop_t_GOV + stop_t_Nor + stop_t_acc + stop_t_sampl; // + stop_t_init;
	cout << "Leftovers...            \t|\tCPU time = " << std::setprecision(5) << stop_t - leftover_time << "\t|\t" << std::setprecision(5) << 100 * (stop_t - leftover_time) / stop_t << " % (Total)" << endl;

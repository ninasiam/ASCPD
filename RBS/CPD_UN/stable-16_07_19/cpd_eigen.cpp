...
int main(int argc, char **argv){
	...
	MatrixXd W_C(BZ(2), R);										// MTTKRP for the computation of cost function
	...
	create_subfactors(A, A_sub, X_A, X_A_sub, BZ, rand_indices, B_cal, 1, 1);
	...
	// MatrixXd new_A(I, R);
	// new_A.setZero();
	// merge_Factors(new_A, A_sub, BZ, B_cal, 1);
	// std::cout << new_A << "\n\n";
	...
	while(1){	//	<-------------------------------------------	Begin AO Algorithm		------------------------------------------>	//
		...
		//	<------------------------		 Update A		------------------------------->	//
		// create_subfactors(A, A_sub, BZ, rand_indices, B_cal, AO_iter, 1);
		// create_subfactors(B, B_sub, BZ, rand_indices, B_cal, AO_iter, 2);
		// create_subfactors(C, C_sub, BZ, rand_indices, B_cal, AO_iter, 3);
		
		start_t_KCB = tic();
		Khatri_Rao_Product(omp_var, C, B, KhatriRao_CB); // C kr B
		stop_t_KCB += toc(start_t_KCB);
		start_t_Wj1 = tic();
		Workerjob(omp_var, BZ(1), BZ(2), A_sub, C_T_C, B_T_B, X_A, KhatriRao_CB, 1);
		...
		//	<------------------------		 Update B		------------------------------->	//
		start_t_KCA = tic();
		Khatri_Rao_Product(omp_var, C, A, KhatriRao_CA); // C kr A
		stop_t_KCA += toc(start_t_KCA);
		start_t_Wj2 = tic();
		Workerjob(omp_var, I, K, B, C_T_C, A_T_A, X_B, KhatriRao_CA, 2);
		...
		//	<------------------------		 Update C		------------------------------->	//
		start_t_KBA = tic();
		Khatri_Rao_Product(omp_var, B, A, KhatriRao_BA); // B kr A
		stop_t_KBA += toc(start_t_KBA);
		start_t_Wj3 = tic();
		Workerjob_C(omp_var, I, J, C, W_C, B_T_B, A_T_A, X_C, KhatriRao_BA);

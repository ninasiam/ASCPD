...
void create_subfactors(const MatrixXd &Factor, MatrixXd &subFactor, const MatrixXd &Matr_Tensor, MatrixXd &Matr_subTensor, const VectorXi &block_size, MatrixXi &rand_indices, MatrixXi &B_cal, int iter, int factor)
{
    ...
    if(factor!=1)    // factor A, C
    {
        for (int i = 0; i < block_size(dim[0]); i++)
        {
            for (int j = 0; j < block_size(dim[1]); j++)
            {
                for (int k = 0; k < block_size(dim[2]); k++)
                {
                    Matr_subTensor(i, (k * block_size(dim[1])) + j) = Matr_Tensor(B_cal(dim[0], i), B_cal(dim[1], j) + (B_cal(dim[2], k) * rand_indices.cols()));
